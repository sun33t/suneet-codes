---
isPublished: true
title: Linting + Formatting + Conventional Commits for Node Projects
author: Suneet Misra
date: 2025-01-27
...
coverImage: laptop_screen_with_js_lfvx2i
keywords:
  [
    "javascript",
    "typescript",
...
<SuspendedArticleImage
  src="laptop_screen_with_js_lfvx2i"
  alt="laptop screen displaying source code"
/>

## What are we making?

We're going to setup a Node.js project with a linting and formatting automation that will use `git hooks` to act upon staged file when we `git commit`. Although we're using [**Next.js**](https://Next.js.org/docs) in this article, the automation that we're building is completely applicable to any and all Node.JS projects. In fact, it's not even limited to Node.js. We can use it with deno and bun projects as well by using the corresponding commands provided by their package managers!

As an added extra, we can also use the approach in this article to implement a library that allows us to standardise commit messages using `conventional-commit` syntax. There'll be a future article diving deeper into the concept of conventional commits and the potential it may have to add value to projects, but for now, we'll go through an optional example of how to set it up.
...
- [**husky**](https://github.com/typicode/husky): A tool that allows us to add git lifecycle hooks to our repo. We could make use of the hooks provided by the `.git` directory within the project folder but using husky allows us to have our hooks committed so that they are portable with the project.
...
![Animated gif demonstrating the gitmoji-cli in action](https://res.cloudinary.com/suneet-codes/image/upload/v1738223286/suneet-codes/articles/linting-formatting-setup-01_gkeocp.gif)
