...
We're going to setup a Node.js project with a linting and formatting automation that will use `git hooks` to act upon staged files when we `git commit`. Although we're using [**Next.js**](https://Next.js.org/docs) in this article, the automation that we're building is completely applicable to any and all Node.JS projects if the main steps are followed and project specific configuration files are used.
