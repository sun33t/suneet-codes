...
We're not using husky here as we did in the last article as husky is primarily designed with the structure of a node project in mind. Instead, we're going to use git's native `core.hooksPath` api to tell git where to look for hook scripts.
...
Then, lets create our `prepare-commit-msg` hook:
