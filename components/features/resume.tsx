...
const RoleComp = ({ role }: { role: Role }) => {
	...
	return (
		<li className="group relative">
			<div className="-inset-x-2 -inset-y-2 absolute flex scale-95 bg-zinc-50 opacity-0 transition group-hover:scale-100 group-hover:opacity-100 sm:rounded-xl dark:bg-zinc-800/50" />
		...
		<Card className="p-6 shadow-none">
			...
			<CardContent className="p-0">
				<ol className="mt-6 space-y-4">
					{roles.map((role) => (
						<RoleComp key={role.company} role={role} />
					))}
				</ol>
				<Link
