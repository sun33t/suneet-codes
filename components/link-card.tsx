...
const LinkCardHoverEffect = memo(() => {
	return (
		<div className="-inset-x-4 -inset-y-6 sm:-inset-x-6 absolute z-0 scale-95 rounded-2xl bg-zinc-50 opacity-0 transition group-hover:scale-100 group-hover:opacity-100 dark:bg-zinc-800/50" />
	);
});
...
const LinkCardInnerContainer = memo(
	({ children, ...rest }: React.HTMLAttributes<HTMLSpanElement>) => {
		return (
			<span {...rest} className="relative z-10" id="card-inner">
				{children}
			</span>
		);
	},
...
export const LinkCardEyebrow = memo(
	({ children, className, ...rest }: React.HTMLAttributes<HTMLDivElement>) => {
		const classes = cn(
			"relative pl-3.5 text-muted-foreground text-sm",
			className,
		);
		return (
